...
def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    ...
    # calculate x and y coordinates to paint hte app centered on the user's screen
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0 and len(var_email) > 0: # enforce the user to provide bot
        ...
        with conn:
            ...
            chkName = count
            if chkName == 0: # if this is - then there is no existence of the fullname and we can add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        if count > 1:
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at this time. \n\nPlease add another record first before you can close")
    conn.close()
...
def onUpdate(self):
    try:
        var_select = se;f.lstList1.curselection()[0] # index of the list selection
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            cur = conn.cursor()
            # count records to see if the user's canges are already in the database... meaning there are no changes to update.
